...
const navigateToUserSettings = () => {
  router.push('/setting/admin/usermanagement');
};
</script>

<template>
  <v-app>
    <Header :componentName="t('adminSettingsTitle')" />
    <v-main class="main-content">
      <v-container>
        <v-row>
          <v-col cols="12" md="6">
            <v-card class="d-flex flex-column fill-height" @click="navigateToUserSettings">
              <v-card-title><v-icon>mdi-account</v-icon> {{ t('userManagementSettingsCardTitle') }}</v-card-title>
              <v-card-subtitle>{{ t('userManagementSettingsCardSubtitle') }}</v-card-subtitle>
            </v-card>
          </v-col>
        </v-row>
      </v-container>
